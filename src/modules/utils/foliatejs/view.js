import * as CFI from '/public/foliate/epubcfi.js'
...
const makeZipLoader = async (file) => {
  const { configure, ZipReader, BlobReader, TextWriter, BlobWriter } =
    await import('/public/foliate/zip.js')
...
export const makeBook = async (file) => {
  ...
  if (file.isDirectory) {
    ...
    const { EPUB } = await import('/public/foliate/epub.js')
    book = await new EPUB(loader).init()
  } else if (!file.size) throw new NotFoundError('File not found')
  else if (await isZip(file)) {
    const loader = await makeZipLoader(file)
    if (isCBZ(file)) {
      const { makeComicBook } = await import('/public/foliate/comic-book.js')
      book = makeComicBook(loader, file)
    } else if (isFBZ(file)) {
      const { makeFB2 } = await import('/public/foliate/fb2.js')
    ...
    const { makePDF } = await import('/public/foliate/pdf.js')
    book = await makePDF(file)
  } else {
    const { isMOBI, MOBI } = await import('/public/foliate/mobi.js')
    if (await isMOBI(file)) {
      const fflate = await import('/public/foliate/fflate.js')
